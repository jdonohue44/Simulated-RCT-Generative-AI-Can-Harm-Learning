...
tbl_real <- format_main_regression_table(regs_real)
kable(tbl_real, digits = 3,
      col.names = c("Outcome", "Coefficient", "Estimate", "SE", "95% CI Lower", "95% CI Upper", "p-value"))
```

The coefficients match the published results: GPT Base improves assisted scores by 0.137 and harms unassisted scores by 0.054; GPT Tutor improves assisted scores by 0.361 with negligible effect on unassisted (-0.004).
...
```{r real-balance, message=FALSE, warning=FALSE, fig.width=7, fig.height=5}
bal_real <- compute_balance(df_real)
plot_love_plot(bal_real)
```

Balance is generally good: all SMDs are below 0.2, and most are below 0.1. Prior GPA shows the largest imbalance (SMD ≈ -0.05 to -0.11), but the regression controls for it.

## Moderator Analysis
We test heterogeneity by prior GPA (median split) and prior ChatGPT use (any vs. none). The key interaction is GPT Base × High GPA on assisted scores: the effect is smaller for high-GPA students (-0.077, p = 0.002), suggesting GPT Base helps lower-achieving students more on assisted tasks. The unassisted harm does not differ significantly by GPA or ChatGPT use.
...
kable(tbl_mod %>% select(moderator, outcome, term_label, estimate, std.error, p.value) %>%
        mutate(estimate = round(estimate, 3), std.error = round(std.error, 3),
               p.value = ifelse(p.value < 0.001, "<0.001", round(p.value, 3))),
      col.names = c("Moderator", "Outcome", "Term", "Estimate", "SE", "p-value"))
```
