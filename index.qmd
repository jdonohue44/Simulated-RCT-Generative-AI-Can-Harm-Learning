---
title: "Generative AI Can Harm Learning: A Simulated Randomized Controlled Trial"
subtitle: "Updated in March, 2026 to incorporate author-shared data"
author: Jared Donohue (jjd2203@columbia.edu)
date: 2024-12-15
format:
  html:
    toc: true
...
# Executive Summary
In this report, we simulate the randomized controlled trial (RCT) reported in Bastani et al. (2025), "Generative AI without guardrails can harm learning: Evidence from high school mathematics," published in the Proceedings of the National Academy of Sciences (PNAS). Our replicated experiment confirmed that having access to Chat GPT-4 improves student performance on math tests, but actually reduces performance when access to Chat GPT is taken away, suggesting it hinders the long-term learning of new concepts. Conversely, using a customized Chat GPT-4 "Tutor" model boosted performance on assisted tasks without significantly affecting unassisted ones, indicating that purpose-built AI models may be a valuable tool for supporting long-term learning. 
...
The independent variables GPTBasec and GPTTutorc indicate the treatment group for each class. The model controlled for prior student performance using normalized GPA from the previous year, PrevGPAi, and included fixed effects for session, grade, year, and time-related variations (Ts, Dg, Ay, Gt). In our simulation, we removed prior GPA and the fixed effects from both the data-generating process and the regression model; because treatment is randomized, these controls affect precision rather than identification. The full specification, including prior GPA and the fixed effects, is estimated on the author-shared data in the validation section below.
...
We did not simulate the session, grade, teacher, and grader identifiers that the original study used as fixed effects, and neither those fixed effects nor prior GPA enter score generation. Because treatment is randomized at the classroom level, these variables affect precision rather than identification, and omitting them lets the simulation focus on the treatment effects reported in the paper. Prior GPA is still generated as a student-level covariate so the simulated dataset mirrors the structure of the real one.

Finally, student scores were simulated, as described in the Simulation of Student Scores section below.
...
Because the paper reports classroom-clustered standard errors, we simulate at the level the inference was performed: each classroom receives its own treatment effect coefficients, drawn from normal distributions centered on the paper's reported point estimates with standard deviations equal to the reported standard errors. A student's score is then the control-group mean, plus the classroom's treatment coefficient for its assigned arm, plus a small amount of individual-level noise (SD = 0.05).

One consequence of this design is worth stating explicitly: the simulation targets the sampling distribution of the treatment-effect estimator, not the raw outcome distribution. Simulated scores vary far less than real scores (the real control-group SD is roughly 0.28, versus 0.05 individual noise here) because between-student variance unrelated to treatment is not modeled, and the conventional OLS standard errors in the simulated regressions are correspondingly smaller than the paper's clustered standard errors. This is a deliberate choice — it anchors the simulated treatment estimates to the paper's reported values — but it means the simulated data should not be used to study outcome dispersion.
...
# Results
In the original study, use of the chatbots increased performance on the assisted assessment, with GPT Base improving scores by .137 (out of 1) and GPT Tutor improving scores by .361 (out of 1) relative to the control group. On the unassisted assessment, GPT Base decreased performance by .054 (out of one) relative to the control group (a 17% decrease), while GPT Tutor's impact on the unassisted portion was small (-.004) and not statistically significant.

Our simulation yielded closely aligned results. On the assisted assessment, GPT Base improved scores by 0.130 (SE = 0.005) and GPT Tutor by 0.365 (SE = 0.005). On the unassisted assessment, GPT Base decreased scores by 0.057 (SE = 0.004), while GPT Tutor's coefficient of -0.005 (SE = 0.004) was not statistically significant (p = 0.275).

The p-values of our two-sample t-tests align with the original study's findings: GPT Base improved assisted scores and decreased unassisted scores with statistical significance (p < 0.001), and GPT Tutor did not have a statistically significant impact on unassisted scores (p = 0.256). All differences between assisted and unassisted scores were statistically significant (p < 0.001).
...
The authors of Bastani et al. (2025) released `final_data.csv` for reproducibility. We re-ran the paper's main regression, checked covariate balance, and conducted a simple moderator analysis. This section validates that our simulation aligns with the real-data results and provides evidence on balance and heterogeneity.

## Main Regression (Reproduced)
We replicate the paper's specification: Part 2 (assisted) and Part 3 (unassisted) scores regressed on GPT Base, GPT Tutor, prior GPA, and fixed effects for teacher, session, grader, and year. Standard errors are clustered by class, and p-values use cluster-level degrees of freedom (number of classes minus one), a conservative choice for cluster-randomized inference.
...
- **GPT Base significantly worsens unassisted scores** (Part 3) by 0.054 points (p = 0.020), a ~17% penalty relative to the control group mean. This is the "crutch effect": students who relied on an unrestricted AI assistant performed worse once the tool was removed.
- **GPT Tutor has no statistically significant effect on unassisted scores** (−0.004, p = 0.749). Its guided design preserved autonomous problem-solving ability even as it boosted assisted performance.
